...
### Ack
This message is sent from the server to the client when a generic message
...
- CreationFailedNicknameTaken: the game has failed its creation because
the host's nickname is already used.
...
This message is sent from the server to the client when a new game couldn't be created
because the host's nickname is used.
...
- ConnectionFailedNicknameTaken: the connection to the game has failed because
  the host's nickname is already used.
- ConnectionFailedGameNotExisting: the connection to the game has failed because
  there isn't any game related to that ID.
...
This message is sent from the server to the client when it's impossible to connect to a game
because the nickname has already been taken.
...
This message is sent from the server to the client when it's impossible to connect to a game
because there isn't any game related to that ID.
...
```mermaid
sequenceDiagram
Client ->> Server: Hello Bob, how are you?
